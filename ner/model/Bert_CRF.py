...
# from da import START_TAG, STOP_TAG, label2idx
...
class BertForNER(BertPreTrainedModel):
    def __init__(self, config, label2idx, start_tag, stop_tag):
        ...
        self.transitions = nn.Parameter(
            torch.randn(self.label_size, self.label_size))  # 转移矩阵，表示从某一列的label转移至某一行的label的TransitionScore
        self.transitions.data[label2idx[self.start_tag], :] = -10000
        self.transitions.data[:, label2idx[self.stop_tag]] = -10000
    ...
    def get_total_scores(self, frames, real_lengths):
        ...
        alpha[:, self.label2idx[self.start_tag]] = 0.  # 初始状态的EmissionScore. START_TAG是0, 其他都是很小的值 "-10000"
        ...
        alpha_ = log_sum_exp(
            alpha_.unsqueeze(-1) + 0 + self.transitions[[self.label2idx[self.stop_tag]], :].T).flatten()  # [batch size]
        return alpha_
    ...
    def get_golden_scores(self, frames, labels_idx_batch, real_lengths):
        ...
        labels = torch.cat(
            [torch.full([labels_idx_batch.shape[0], 1], self.label2idx[self.start_tag], dtype=torch.long).to(device),
             labels_idx_batch], dim=1)  # [batch size, seq len+1],注意不要+[STOP_TAG]; 结尾有处理
        index = 0
        for i in range(frames.shape[1]):  # 沿途累加每一帧的转移和发射
            index += 1
            frame = frames[:, i, :]  # [batch size, label_size]
            score += self.transitions[labels[:, i + 1], labels[:, i]] + frame[
                range(frame.shape[0]), labels[:, i + 1]]  # [batch size]

            for idx, length in enumerate(real_lengths):
                if length == index:
                    score_[idx] = score[idx]

        score_ = score_ + self.transitions[self.label2idx[self.stop_tag], labels[:, -1]]  # [batch size],加上到STOP_TAG的转移
        return score_
    ...
    def viterbi_decode(self, frames):
        ...
        alpha[0][self.label2idx[self.start_tag]] = 0
        for frame in frames:
        ...
        # 回溯路径
        smat = alpha.T + 0 + self.transitions[[self.label2idx[self.stop_tag]], :].T
        val, idx = torch.max(smat, 0)
