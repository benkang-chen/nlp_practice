...
class BertForRE(BertPreTrainedModel):
    def __init__(self, config, start_tag, stop_tag, tag2idx, idx2tag, relation2idx, idx2relation):
        ...
        self.transitions = nn.Parameter(
            torch.randn(self.tag_size, self.tag_size))  # 转移矩阵，表示从某一列的tag转移至某一行的tag的TransitionScore
    ...
    def get_total_scores(self, frames, real_lengths):
        ...
        alpha[:, self.tag2idx[self.start_tag]] = 0.  # 初始状态的EmissionScore. START_TAG是0, 其他都是很小的值 "-10000"
        ...
        for frame in frames:
            ...
            alpha = log_sum_exp(alpha.unsqueeze(-1) + frame.unsqueeze(1) + self.transitions.T).squeeze(
                1)  # [batch size, tag_size]
        ...
        alpha_ = log_sum_exp(
            alpha_.unsqueeze(-1) + 0 + self.transitions[[self.tag2idx[self.stop_tag]], :].T).flatten()  # [batch size]
        return alpha_
    ...
    def get_golden_scores(self, frames, tag_ids_batch, real_lengths):
        ...
        tags = torch.cat([torch.full([tag_ids_batch.shape[0], 1], self.tag2idx[self.start_tag], dtype=torch.long).to(device),
                          tag_ids_batch], dim=1)  # [batch size, seq len+1],注意不要+[STOP_TAG]; 结尾有处理
        index = 0
        for i in range(frames.shape[1]):  # 沿途累加每一帧的转移和发射
            ...
            score += self.transitions[tags[:, i + 1], tags[:, i]] + frame[
                range(frame.shape[0]), tags[:, i + 1]]  # [batch size]

            for idx, length in enumerate(real_lengths):
                if length == index:
                    score_[idx] = score[idx]
        score_ = score_ + self.transitions[self.tag2idx[self.stop_tag], tags[:, -1]]  # [batch size],加上到STOP_TAG的转移
        return score_
    ...
    def viterbi_decode(self, frames):
        ...
        alpha[0][self.tag2idx[self.start_tag]] = 0
        for frame in frames:
        ...
        # 回溯路径
        smat = alpha.T + 0 + self.transitions[[self.tag2idx[self.stop_tag]], :].T
        val, idx = torch.max(smat, 0)
    ...
    def forward(self, input_ids, attention_mask, tag_ids, sub_mask, obj_mask, labels, real_lengths):
    ...
    def predict(self, text, chars, input_ids, attention_mask):
        ...
        pred_tags = [self.idx2tag[ix] for ix in res_ner[1]]
            ...
            for obj in objects:
                if obj != sub:
                    try:
                        ...
                        logits = self.get_relation_logit(pooled_output, hidden_output, sub_mask,
                                                         obj_mask)  # [1, tagset_size]
                        relation = self.idx2relation[logits.argmax().item()]
